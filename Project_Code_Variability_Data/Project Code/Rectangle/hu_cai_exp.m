function [k,H,time,C,k_i,Q_final,Q_initial,k_final,Q_flow_direction_final,changed_flow_direction]= hu_cai_exp(tspan,c_0,s,t,source_loc,sink_loc,source_boundary_conditions,sink_boundary_conditions,weights)

num_edges = length(s);
weights = ones(1,num_edges) + .5*exprnd(0,1,num_edges);
...
Q_initial = calculate_flows(k,I,it,F,p,num_nodes);
positive  = Q_initial > 0;
negative = (Q_initial < 0)*-1;
Q_flow_direction_initial = positive + negative;
k_i = k;
[time,C] = ode45(@(t,c)adaptation_ode(t,c,c_0,I,it,F,p,num_nodes),tspan, k_i);
k_final = C(end,:)';
