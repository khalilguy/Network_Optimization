...
total_init_Q_matrix = zeros(length(Q),5000000);
...
while found < target || found_flow < target
    for i = 1:trials
        [~,~,~,~,Q,Q_initial,k,~,Qflow,changed_direction] = hu_cai_model_with_fixed_sinks_and_sources(size_graph_x, size_graph_y,source_loc,sinks,source_bound,tspan,c_0,weights,m,v);
        used_edges = 0;
        E = 0;
...
%         end
        ...
        total_init_Q_matrix(:,i+total_trials) = Q_initial;
...
while found < target
    ...
    for i = 1:trials
        ...
        total_k_matrix(:,i+total_trials) = k;
        total_Q_matrix(:,i+total_trials) = Q;
        total_init_Q_matrix(:,i+total_trials) = Q_inital;
        total_used_edges(i + total_trials,1) = used_edges;
        total_energy_array(i+total_trials,1) = E;
    end
...
total_init_Q_matrix(:,~any(total_init_Q_matrix,1)) = [];
total_used_edges(total_used_edges == 0) = [];
total_energy_array(total_energy_array == 0) = [];
trials_array(trials_array == 0) = [];
flow_trials_array(flow_trials_array == 0) = [];
...
% path = ['hu_cai_raw_data_plots/' int2str(size_graph_x) '_by_' int2str(size_graph_y) '_' int2str(length(source_loc)) 'source_' int2str(length(sinks)) 'sinks_' num2str(m) 'm_' num2str(v) 'v'];
% mkdir(path);
...
filename = ['small_network_data/gd_hu_cai' int2str(size_graph_x) '_by_' int2str(size_graph_y) '_' int2str(length(source_loc)) 'source_' int2str(length(sinks)) 'sinks_' num2str(m) 'm_' num2str(v) 'v.mat'];
